...
7. **Optional — SharePoint:** call **`POST /api/rewst/sharepoint/upload`** with a **direct JSON body** (`site_id`, `folder_path`, `file_name`, `content_base64`, optional `overwrite` / `content_type`), plus Graph app settings on the Function App (see [SETUP.md](./SETUP.md)).
...
- Schema **`minLength: 2`** on `payload_json` rejects a zero- or one-character string only; real payloads are always longer. (Applies to validate/render routes.)
